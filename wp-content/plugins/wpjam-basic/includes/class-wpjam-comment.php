...
class WPJAM_Comment{
	...
	public static function filter_post_json($post_json, $post_id){
		...
		if(is_singular($post_type)){
			...
			$current_user	= wpjam_get_current_user(false);
			$current_user	= ($current_user && !is_wp_error($current_user)) ? $current_user : null;
				...
				if($action == 'like'){
					...
					if($likes && $current_user){
						if(!empty($current_user['user_id'])){
							...
							$emails		= wp_list_pluck($likes, 'email');
							if($emails && in_array($user_email, $emails)){
								$is_liked	= 1;
							}
						}
					}
					...
					if($favs && $current_user){
						if(!empty($current_user['user_id'])){
							...
							$user_email	= $current_user['user_email'];
							$emails		= wp_list_pluck($favs, 'email');
	...
	public static function insert($comment_data){
		...
		$current_user	= $comment_data['current_user'] ?? '';
		...
		if($user_id){
			...
			return new WP_Error('invalid_current_user', '无效的 current_user');
		}
	...
	public static function action($current_user, $post_id, $action='like'){
		...
		if($comments){
			...
			if($status == 1){
				$result	= self::insert(compact('current_user', 'post_id', 'type'));
			}else{
